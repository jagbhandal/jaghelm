...
export const UPSCard = React.memo(function UPSCard({ upsData, borderColor, banner }) {
  ...
  return (
    <div
      ...
      style={{ borderTop: `2px solid ${borderColor || 'var(--green)'}`, ...sectionBgStyle(sec) }}
    >
      <div className="section-header grab-handle">
        ...
        <div>
          <div className="section-title">{sec.title || 'UPS Power'}</div>
      ...
      <div className="ups-grid">
        <div className="metric-block">
          ...
          <span
            ...
            style={{
              fontSize: 18,
              color: upsData?.status != null ? sC(upsData.status) : 'var(--text-muted)',
            }}
          >
            {sL(upsData?.status)}
          </span>
        ...
        <div className="metric-block">
          ...
          <span className="metric-value">
            {upsData?.charge != null ? Math.round(upsData.charge) : '—'}
          ...
          {upsData?.charge != null && (
            <div className="metric-bar">
              <div
                ...
                style={{
                  width: `${upsData.charge}%`,
                  background: upsData.charge > 50 ? 'var(--green)' : 'var(--amber)',
                }}
              />
            </div>
          )}
        ...
        <div className="metric-block">
          ...
          <span className="metric-value">
            {upsData?.load != null ? Math.round(upsData.load) : '—'}
            <span className="metric-unit">%</span>
          </span>
          {upsData?.load != null && (
            <div className="metric-bar">
              <div
                ...
                style={{
                  width: `${upsData.load}%`,
                  background: upsData.load > 80 ? 'var(--red)' : 'var(--green)',
                }}
              />
            </div>
          )}
...
export const GiteaActivity = React.memo(function GiteaActivity({ commits, banner }) {
  ...
  return (
    <div
      className="glass-card node-card"
      style={{ borderTop: '2px solid var(--accent)', ...sectionBgStyle(sec) }}
    >
      <div className="section-header grab-handle">
        <div
          className="section-icon"
...
export const CronJobs = React.memo(function CronJobs({ nodes, banner }) {
  ...
  return (
    <div
      className="glass-card node-card"
      style={{ borderTop: '2px solid var(--teal)', ...sectionBgStyle(sec) }}
    >
      <div className="section-header grab-handle">
        <div
          className="section-icon"
          style={{ background: 'rgba(23,146,153,0.1)', border: '1px solid rgba(23,146,153,0.2)' }}
        >
          {renderIcon(sec.icon || '⏱')}
        </div>
        <div>
...
export const QuickLaunch = React.memo(function QuickLaunch({ borderColor }) {
  const { config } = useConfig();
  const sec = config?.sections?.quicklaunch || {};
  const groups = [
    { key: 'personal', label: 'Personal' },
    { key: 'management', label: 'Management' },
    { key: 'devops', label: 'Dev & Monitoring' },
  ];
  return (
    <div
      className="glass-card node-card"
      style={{ borderTop: `2px solid ${borderColor || 'var(--blue)'}`, ...sectionBgStyle(sec) }}
    >
      <div className="section-header grab-handle">
        <div
          className="section-icon"
        ...
        <div>
          <div className="section-title">{sec.title || 'Quick Launch'}</div>
        </div>
      </div>
