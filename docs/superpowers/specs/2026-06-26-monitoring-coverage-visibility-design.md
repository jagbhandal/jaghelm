...
## Live-Kuma Verification (T7, 2026-06-27)

Probed the live status-page API (`/api/status-page/default` + `/api/status-page/heartbeat/default`) read-only. **Finding:** every monitor reports `active: null` on this endpoint — so `pub.active !== false` (which the original design leaned on to hide paused/retired monitors) is a **no-op** there; it can't distinguish a paused monitor. Heartbeats, however, carry a `time` field, giving a reliable signal `active` does not.

**Resolution — heartbeat-staleness guard (replaces the `active`-based fail-safe for outage synthesis):** a monitor reporting `down` is synthesised into a red card only if its **latest heartbeat is fresh** (`now − lastBeatAt ≤ MONITOR_STALE_MS`, default 10 min, `JAGHELM_MONITOR_STALE_MS`-overridable). A genuinely-down service is checked every interval and beats fresh `down`; a paused/retired monitor stops beating and goes stale → excluded, so it can never leave a phantom red card after its container is gone — correct **regardless** of whether Kuma keeps paused monitors on the status page. A monitor whose beat time is missing/unparseable is treated as fresh (we only exclude on a *positively*-stale beat, never miss a real outage over a parse gap). `active !== false` is retained as a cheap belt-and-suspenders. Beat times are parsed as UTC (`"YYYY-MM-DD HH:mm:ss.SSS"`, no zone). This closes the §10 open item.
